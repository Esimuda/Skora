...
export class ResultsController {
  ...
  findOne(
    @Param('schoolId') schoolId: string,
    @Param('classId') classId: string,
    @Query('term') import { Body, Controller, Get, Param, Post, Put, Query, Request, UseGuards } from '@nestjs/common';
import { ApiTags, ApiBearerAuth } from '@nestjs/swagger';
import { JwtAuthGuard } from '../common/guards/jwt-auth.guard';
import { RolesGuard } from '../common/guards/roles.guard';
import { Roles } from '../common/decorators/roles.decorator';
...
export class ResultsController {
  ...
  async computed(
    ...
    // Teachers get a plain array — no watermark logic needed
    if (role === 'teacher') {
      return data;
    }

    // Principal/admin preview gets watermark flag
    if (!isPortal || !pinUseId) {
