 ...
 const handleSaveAll = async () => {
    ...
    if (incomplete.length > 0) {
      const names = incomplete.map((s) => {
        ...
        return student ? `${student.lastName} ${student.firstName}` : s.studentId;
      });
      setApiError(
        `Cannot save — ${incomplete.length} student${incomplete.length > 1 ? 's' : ''} ` +
        `have incomplete scores: ${names.join(', ')}. ` +
        `All three fields (CA1, CA2, Exam) must be filled for every student.`
      );
      return;
    }
