...
import { useDataStore, ComputedStudentResult } from '@/store/dataStore';
...
export const CommentsPage = () => {
  const { classes, getStudentsByClass, saveComment, getComment, computeClassResults } = useDataStore();
  ...
  const [classResults, setClassResults] = useState<ComputedStudentResult[]>([]);
  ...
  useEffect(() => {
    ...
    const results = computeClassResults(selectedClassId, CURRENT_TERM, CURRENT_YEAR);
    setClassResults(results);
    const saved = new Set<string>();
    getStudentsByClass(selectedClassId).forEach((s) => {
      const c = getComment(s.id, selectedClassId, CURRENT_TERM, CURRENT_YEAR);
      if (c?.teacherComment?.trim()) saved.add(s.id);
    });
  ...
  useEffect(() => {
    if (!selectedStudentId || !selectedClassId) { setCommentText(''); return; }
    const existing = getComment(selectedStudentId, selectedClassId, CURRENT_TERM, CURRENT_YEAR);
  ...
  const getSuggestedComment = (result: ComputedStudentResult) => {
  ...
  const classTotal = getStudentsByClass(selectedClassId).length;
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <div>
          ...
          <p className="text-on-surface-variant text-sm mt-1">
            Write a comment for each student — appears on their result sheet
          </p>
        </div>
            ...
            <div className="ledger-card p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-sm text-on-surface-variant">
                  {savedStudents.size} of {classTotal} students commented
                </span>
              </div>
              <div className="h-1.5 bg-surface-container-highest rounded-full overflow-hidden">
                <div
                  className="h-full bg-secondary rounded-full transition-all duration-500"
                  style={{ width: classTotal > 0 ? `${(savedStudents.size / classTotal) * 100}%` : '0%' }}
                />
              </div>
            </div>
              ...
              <div className="ledger-card overflow-hidden">
                ...
                {classResults.length === 0 ? (
                  <div className="p-6 text-center text-on-surface-variant text-sm">
                    ...
                    {getStudentsByClass(selectedClassId).length > 0 && (
                          ...
                          <div className="flex items-center justify-between">
                            <div className="min-w-0">
                              ...
                              <p className="text-xs text-on-surface-variant mt-0.5">
                                {result.percentage.toFixed(1)}% · {result.student.admissionNumber}
                              </p>
                            </div>
                            {isSaved && (
                              <Icon name="check_circle" className="text-secondary text-base flex-shrink-0 ml-2" />
                            )}
                          </div>
                        </button>
                      );
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-3 mt-1 flex-wrap">
                            <span className="text-xs font-bold px-2.5 py-1 rounded-full bg-primary/5 text-primary">
                              {formatPosition(selectedResult.position)} Position
                            </span>
                            <span className="text-xs text-on-surface-variant">
                              {selectedResult.percentage.toFixed(1)}%
                            </span>
                            <span className="text-xs text-on-surface-variant">
                              {selectedResult.totalScore}/{selectedResult.totalPossible} marks
                            </span>
                            <span className="text-xs text-on-surface-variant">
                              Class of {selectedResult.totalStudents}
                            </span>
                          </div>
                        </div>
                        <button
                          ...
                          className={`btn-primary text-sm flex items-center gap-2 disabled:opacity-60 flex-shrink-0 ${
                            saveStatus === 'saved' ? 'from-secondary to-secondary bg-secondary' : ''
                          }`}
                        >
                    ...
                    <div className="p-5 space-y-5">
                      ...
                      <div className="p-4 bg-surface-container-low rounded-xl">
                        <p className="text-xs font-bold text-on-surface-variant uppercase tracking-widest mb-2">
                          Suggested based on performance
                        </p>
                        <p className="text-sm text-on-surface italic mb-3">
                          "{getSuggestedComment(selectedResult)}"
                        </p>
                        <button
                          onClick={() => { setCommentText(getSuggestedComment(selectedResult)); setSaveStatus('idle'); }}
                            ...
                            <button
                              ...
                              className={`w-full text-left text-xs p-3 rounded-xl border-2 transition-colors ${
                                commentText === qc
                                  ...
                                  : 'border-outline-variant/20 hover:border-primary/30 hover:bg-surface-container-low text-on-surface-variant'
                              }`}
                            >
                              "{qc}"
                            </button>
