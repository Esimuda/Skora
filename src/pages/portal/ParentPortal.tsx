...
import type { Term, StudentResult, School, Score } from "@/types";
...
// Build a clean (no watermark) result HTML page for download
function buildCleanResultHTML(
  result: any,
  school: School,
  term: Term,
): string {
  ...
  switch (school.templateId) {
    case "modern":
      markup = renderToStaticMarkup(
        <ModernResultSheet result={studentResult} school={school} watermarked={false} />
      );
      break;
    case "hybrid":
      markup = renderToStaticMarkup(
        <HybridResultSheet result={studentResult} school={school} watermarked={false} />
      );
      break;
    default:
      markup = renderToStaticMarkup(
        <ClassicResultSheet result={studentResult} school={school} watermarked={false} />
      );
  }
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .download-bar { background: #fff; border: 1px solid #e0e0e0; border-radius: 8px; padding: 12px 20px; margin-bottom: 16px; font-size: 13px; color: #333; max-width: 794px; width: 100%; display: flex; align-items: center; justify-content: space-between; gap: 12px; }
    .download-bar button { background: #00113a; color: #fff; border: none; border-radius: 6px; padding: 8px 18px; font-size: 13px; font-weight: 700; cursor: pointer; }
    .download-bar button:hover { background: #002366; }
    @page { size: A4 portrait; margin: 0; }
    @media print { body { background: white; padding: 0; } .download-bar { display: none; } }
  </style>
</head>
<body>
  <div class="download-bar">
    <span>📄 <strong>${result.student.lastName} ${result.student.firstName}</strong> — Official Result</span>
...
function toStudentResult(result: any, term: Term): StudentResult {
  return {
    student: result.student,
    scores: (result.scores ?? []).map((s: Score) => ({
      ...s,
      term,
...
export const ParentPortal = () => {
  ...
  const handleDownload = () => {
    if (!result || !resultSchool) return;
    const html = buildCleanResultHTML(result, resultSchool, term);
  ...
  const getTermName = (t: Term) =>
    t === "first" ? "First" : t === "second" ? "Second" : "Third";
  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col">
      ...
      {/* Result step gets special full-width treatment */}
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="bg-surface-container-low border-b border-outline-variant/20 px-4 py-3 flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* Download button (only on full view) */}
              {viewMode === "full" && (
                <button
                ...
                >
                  <Icon name="download" className="text-sm" /> Download PDF
                </button>
              )}
            </div>
          </div>

          {/* Full result sheet view */}
          {viewMode === "full" && (
          ...
          {/* Summary view — the original card layout */}
          {viewMode === "summary" && (
