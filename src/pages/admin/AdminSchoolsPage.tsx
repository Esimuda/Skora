                ...
                <li key={school.id}>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-on-surface text-sm truncate">{school.name}</p>
                      <p className="text-xs text-on-surface-variant truncate mt-0.5">
                        {school.address}{school.state ? ` · ${school.state}` : ''}
                      </p>
                    </div>

                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full flex-shrink-0 ${typeColor(school.schoolType)}`}>
                      {typeLabel(school.schoolType)}
                    </span>

                    <span className="text-xs text-on-surface-variant flex-shrink-0 hidden sm:block">
                      {new Date(school.createdAt).toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' })}
                    </span>
